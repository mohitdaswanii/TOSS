...
// Staking distributes the TOSS rewards based on staked TOSS to each user.
...
contract Staking is Ownable {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;     // How many TOSS tokens the user has provided.
    ...
    // Address of the TOSS Token contract.
    IERC20 public TOSS;
    // The total amount of TOSS that's paid out as reward.
    ...
    function fund(uint256 _amount) external onlyOwner {
        ...
        TOSS.transferFrom(address(msg.sender), address(this), _amount);
    }
    ...
    function claimReward(uint256 _pid) external {
        ...
        TOSS.safeTransfer(address(msg.sender), pendingAmount);
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        TOSS.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        user.amount = user.amount - _amount;
        pool.stakedAmount = pool.stakedAmount - _amount;
        TOSS.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw(uint256 _pid) external {
        ...
        user.timestamp = block.timestamp;
        user.rewardDebt = 0;
        TOSS.safeTransfer(address(msg.sender), amount);
    ...
    function withdrawFeeAmount(address _to) external onlyOwner {
        ...
        TOSS.safeTransfer(_to, feeAmount);
        feeAmount = 0;
    }
    ...
    function withdrawRewards(address _to) external onlyOwner {
        ...
        TOSS.safeTransfer( _to, rewardsAmount);
        rewardsAmount = 0;
    }

    //Withraw total amount of contract to address _to
    function withdrawFunds(address _to) external onlyOwner {
        uint256 balance = TOSS.balanceOf(address(this));
        TOSS.safeTransfer(_to, balance);
    }
}
